...
describe('TimelineItem', () => {
  ...
  test('title & slot default', () => {
    ...
    expect(wrapper.find('.fx-timeline-item_title').text()).toBe(itemTitle)
    expect(wrapper.find('.fx-timeline-item_content').text()).toBe(itemContent)
  })
  ...
  test('dotColor', () => {
    ...
    expect(wrapper.find('.fx-timeline-item_dot').attributes('style')).toContain(
      `border-color: ${color};`
    )
  })
