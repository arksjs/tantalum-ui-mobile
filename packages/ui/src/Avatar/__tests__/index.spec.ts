...
describe('AvatarGroup', () => {
  ...
  test.concurrent('totalCount', async () => {
    ...
    const $text = $last.find('.ta-avatar-group_count-number')
  ...
  test.concurrent('countColor', async () => {
    ...
    const $last = wrapper.findAllComponents(Avatar).at(-1) as VueWrapper<any>
    expect($last.attributes('style')).toContain('--ta-color: #6667AB;')
    expect($last.attributes('style')).toContain('--ta-front-color: #ffffff;')

    await wrapper.setProps({
      countColor: '#E2C0BF'
    })
    expect($last.attributes('style')).toContain('--ta-color: #E2C0BF;')
    expect($last.attributes('style')).toContain('--ta-front-color: #493234;')
  })
})
